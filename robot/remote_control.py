...
controller = controller_mappings.create_xbox_one_wireless_controller()
    ...
    if controller.is_connected():
        ...
        try:
            if tank_steer:
                ...
                ly = -controller.read_axis("LY") * dampen_speed
                ry = -controller.read_axis("RY") * dampen_speed

                diff = abs(ly - ry)

                if ly > ry:
                    ly = ry + dampen_turns*diff
                else:
                    ry = ly + dampen_turns*diff
          
                tbot.set_left_speed(ly if abs(ly) > threshold else 0)
                tbot.set_right_speed(ry if abs(ry) > threshold else 0)
            else:
